...
async function testRealTimeSystem() {
  debugLog('🧪 Testing real-time system...');
  ...
  try {
    ...
    if (response.status === 200) {
      debugLog('✅ Real-time endpoint is accessible', 'success');
      ...
      if (statsResponse.ok) {
        ...
        debugLog(`📊 Current Stats: ${statsData.data?.todaysOrders || 0} orders today`, 'success');
      }
      
    } else if (response.status === 401) {
      debugLog('❌ Not authorized - please log in as admin', 'error');
    } else {
      debugLog(`❌ Endpoint returned ${response.status}`, 'error');
    }
  } catch (error) {
